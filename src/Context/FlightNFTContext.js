...
import {  mintABITestnet, mintAddressTestnet, 
...
export default function FlightNFTProvider({ children }) {
  ...
  const mintTicketNFTTestnetBNB = async (uriNft) => {
    try {
      if (ethereum) {
  ...
  const mintTicketNFTTestnetUSDSC = async (uriNft) => {
    try {
      if (ethereum) {
        const chainid = await window.ethereum.request({
          method: "eth_chainId",
  ...
  const mintTicketNFTTestnetDSL = async (uriNft) => {
    try {
      if (ethereum) {
        const chainid = await window.ethereum.request({
          method: "eth_chainId",
